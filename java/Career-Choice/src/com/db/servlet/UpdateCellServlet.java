...
public class UpdateCellServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req,
        ...
        try {
            int count = dao.updateCell(column, year, industryType, newValue);
            if (count > 0) {
                resp.getWriter().println(count + "건 수정 완료");
            } else {
                resp.getWriter().println("0건 수정됨(조건 불일치)");
            }
        } catch (SQLException e) {
            throw new ServletException("셀 수정 중 오류", e);
        }
    }
}
